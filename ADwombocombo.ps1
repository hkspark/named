...
try {
    ...
    if ($krbtgt.Enabled) {
        Write-Log "WARNING: krbtgt account is ENABLED – this should never be the case. Investigate immediately." "WARN"
...
if (Prompt-YesNo "Rotate krbtgt password? (Recommended – invalidates any forged Golden Tickets)") {
    ...
    Write-Host "      A 10-minute wait between rotations ensures replication across all DCs." -ForegroundColor Yellow
    ...
    foreach ($round in 1..2) {
        try {
            ...
            if ($round -eq 1) {
                Write-Host "`n  Round 1 complete. You should wait ~10 minutes for AD replication" -ForegroundColor Yellow
                Write-Host "  before performing round 2 to ensure the old hash is fully flushed." -ForegroundColor Yellow
...
foreach ($user in $AllUsers) {
    ...
    # krbtgt should already be disabled – verified and handled in Section 1
    if ($sam -eq 'krbtgt') {
